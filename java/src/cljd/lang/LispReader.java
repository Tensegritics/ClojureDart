...
final static Var READER_RESOLVER = Var.intern(RT.CLOJURE_NS, Symbol.intern("*reader-resolver*"), null);
...
static private final Object PLATFORM_FEATURES = PersistentHashSet.EMPTY;
...
static public Object read(PushbackReader r, boolean eofIsError, Object eofValue, boolean isRecursive, Object opts)
{
	...
	return read(r, eofIsError, eofValue, null, null, isRecursive, opts, null, (Resolver) READER_RESOLVER.deref());
}
...
static private Object read(PushbackReader r, boolean eofIsError, Object eofValue, boolean isRecursive, Object opts, Object pendingForms) {
	return read(r, eofIsError, eofValue, null, null, isRecursive, opts, ensurePending(pendingForms), (Resolver) READER_RESOLVER.deref());
}
...
static private Object installPlatformFeature(Object opts) {
    if(opts == null)
        ...
        if (features == null)
            ...
            return mopts.assoc(LispReader.OPT_FEATURES, features);
    }
}
		...
		if(s.startsWith("::"))
            ...
            if(resolver != null)
                ...
                if(ks.getNamespace() != null)
                    nsym = resolver.resolveAlias(Symbol.intern(ks.getNamespace()));
                else
...
public static class NamespaceMapReader extends AFn{
	public Object invoke(Object reader, Object colon, Object opts, Object pendingForms) {
		...
		if (auto) {
			...
			if (sym == null) {
                if(resolver != null)
                  ns = resolver.currentNS().getName();
                else
				    ns = currentNS().getName().getName();
				...
				if (resolver != null)
                    ...
                    Namespace rns = currentNS().lookupAlias((Symbol)sym);
...
//		if(o instanceof Symbol)
//			{
//			Object v = Compiler.maybeResolveIn(currentNS(), (Symbol) o);
//			if(v instanceof Var)
//				return v;
//			}
		...
		if(isSpecial(form))
			...
			if(sym.getNamespace() == null && sym.getName().endsWith("#"))
                ...
                if(nsym != null){
                    ...
                    sym = Symbol.intern(nsym.getName(), sym.getName());
                    }
                    ...
                    if(rsym != null)
                      ...
                      sym = Symbol.intern(resolver.currentNS().getName(),sym.getName());
                }
				...
				if(sym.getNamespace() != null)
					maybeClass = currentNS().getMapping(
		...
		for(; ;) {
			if(result == READ_STARTED) {
				...
				if (hasFeature(form, opts)) {
					...
					form = read(r, false, READ_EOF, ')', READ_FINISHED, true, opts, pendingForms, (Resolver) READER_RESOLVER.deref());
