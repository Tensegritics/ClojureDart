...
Future eval(x) async {
  ...
  //try {
    out.write("import 'package:flutter/foundation.dart';\nimport 'dart:io';\nFuture exec() async {\n  return ");
    emit(x, out);
    out.write(";\n}\n");
  //} //finally {
    //await out.close();
...
// function to call in the main() of a flutter app

Future startRepl() async {
  // StreamController is fed by the extension callback bellow
  // TODO: Some abstraction would be great
  final controller = StreamController<String>();
  dev.registerExtension('ext.clojure.read', (String method, Map<String, String> parameters) {
      controller.add(parameters['stream']);
      return Future.value(dev.ServiceExtensionResponse.result(json.encode({'response': 'Success'})));
  });

  dev.registerExtension('ext.clojure.eval', (String method, Map<String, String> parameters) async {
      StringBuffer sb = StringBuffer();
      clj_print(await evalexpr.exec(), sb);
      return Future.value(dev.ServiceExtensionResponse.result(json.encode({'response': sb.toString()})));
  });
  initMacros();
  final rdr = ReaderInput(controller.stream);
  try {
    while(true) {
      final expr = await read(rdr);
      final emit = await eval(expr);
      // TODO: some abstraction would be great
      // TODO: post emition line by line
      dev.postEvent('clojure.compiler.emit', {'response': emit});
