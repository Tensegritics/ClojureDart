...
(defn ensure-pubspec-dependencies []
  (let [scan-pubspec (fn [filter-line-fn rf]
        ...
        _ (run!
            (fn [old-dep-name]
              (exec {:in nil :out nil} (some-> *deps* :cljd/opts :kind name) "pub" "remove" old-dep-name))
            (scan-pubspec
              (fn [seen-cljd-tag acc line]
                (if seen-cljd-tag
                  (conj acc seen-cljd-tag)
                  acc))
    ...
    (spit "pubspec.yaml"
      (scan-pubspec
        (fn [seen-cljd-tag acc line]
          (if-not seen-cljd-tag (conj acc line) acc))
        (fn [v]
          (clojure.string/join "\n" (concat v ["clojuredart: #Generated by the ClojureDart compiler, please don't edit the blocks below."]
                                      (map #(-> % first (->> (str "  ")) (str ":")) cljd-deps-with-pure-dart-dependencies))))))))
